...
public final class CoreDataProjectRepository: ProjectRepositoryProtocol, @unchecked Sendable {
    ...
    public func fetchAllProjects(completion: @escaping @Sendable (Result<[Project], Error>) -> Void) {
        ...
        viewContext.perform {
            ...
            do {
                ...
                let projects = StateProjectMapper.toDomainArray(from: entities)
    ...
    public func fetchProject(withId id: UUID, completion: @escaping @Sendable (Result<Project?, Error>) -> Void) {
        ...
        viewContext.perform {
            ...
            do {
                let entities = try self.viewContext.fetch(request)
                let project = entities.first.map { StateProjectMapper.toDomain(from: $0) }
    ...
    public func fetchCustomProjects(completion: @escaping @Sendable (Result<[Project], Error>) -> Void) {
        ...
        viewContext.perform {
            ...
            do {
                let entities = try self.viewContext.fetch(request)
                let projects = StateProjectMapper
            ...
            case .success(let isAvailable):
                ...
                self.backgroundContext.perform {
                    do {
                        ...
                        _ = StateProjectMapper.toEntity(from: project, in: self.backgroundContext)
        ...
        viewContext.perform {
            ...
            do {
                ...
                if let existingProject = existingProjects.first {
                    let inboxProject = StateProjectMapper.toDomain(from: existingProject)
                    callback.deliver(.success(inboxProject))
                } else {
                    self.backgroundContext.perform {
                        let inboxProject = Project.createInbox()
                        _ = StateProjectMapper.toEntity(from: inboxProject, in: self.backgroundContext)
        ...
        backgroundContext.perform {
            do {
                ...
                if let selected = self.selectCanonicalInbox(from: inboxCandidates) {
                    ...
                    canonicalInbox = StateProjectMapper.toEntity(from: Project.createInbox(), in: self.backgroundContext)
    ...
    public func updateProject(_ project: Project, completion: @escaping @Sendable (Result<Project, Error>) -> Void) {
        ...
        backgroundContext.perform {
            ...
            if let entity = StateProjectMapper.findEntity(byId: project.id, in: self.backgroundContext) {
                do {
                    ...
                    StateProjectMapper.updateEntity(entity, from: project)
                    try self.backgroundContext.save()
