        ...
        viewContext.perform {
            do {
                ...
                let objects = try V2CoreDataRepositorySupport.fetchObjects(
                    in: self.viewContext,
                    entityName: StateSectionMapper.entityName,
                ...
                completion(.success(try objects.map(StateSectionMapper.validatedDomain)))
            } catch {
    ...
    public func create(_ section: TaskerProjectSection, completion: @escaping @Sendable (Result<TaskerProjectSection, Error>) -> Void) {
        backgroundContext.perform {
            do {
                ...
                _ = StateSectionMapper.apply(normalizedSection, to: object)
