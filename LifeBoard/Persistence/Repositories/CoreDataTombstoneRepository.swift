        ...
        backgroundContext.perform {
            do {
                ...
                let object = try V2CoreDataRepositorySupport.upsertByID(
                    ...
                    entityName: StateTombstoneMapper.entityName,
                    id: tombstone.id
                )
                _ = StateTombstoneMapper.apply(tombstone, to: object)
    ...
    public func fetchExpired(before date: Date, completion: @escaping @Sendable (Result<[TombstoneDefinition], Error>) -> Void) {
        viewContext.perform {
            do {
                ...
                completion(.success(objects.map(StateTombstoneMapper.toDomain)))
            } catch {
                completion(.failure(error))
            }
        }
