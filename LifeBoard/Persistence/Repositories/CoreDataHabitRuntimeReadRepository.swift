...
public final class CoreDataHabitRuntimeReadRepository: HabitRuntimeReadRepositoryProtocol, @unchecked Sendable {
    ...
    private func fetchHabits(predicate: NSPredicate? = nil) throws -> [HabitDefinitionRecord] {
        let request = NSFetchRequest<NSManagedObject>(entityName: StateHabitDefinitionMapper.entityName)
        request.predicate = predicate
        request.sortDescriptors = [
            NSSortDescriptor(key: "createdAt", ascending: true),
            NSSortDescriptor(key: "id", ascending: true)
        ]
        return try context.fetch(request).map(StateHabitDefinitionMapper.toDomain)
    }
