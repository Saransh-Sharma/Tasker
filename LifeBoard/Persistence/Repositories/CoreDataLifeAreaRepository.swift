        ...
        viewContext.perform {
            do {
                let objects = try V2CoreDataRepositorySupport.fetchObjects(
                    in: self.viewContext,
                    entityName: StateLifeAreaMapper.entityName,
                    sort: [
                        NSSortDescriptor(key: "sortOrder", ascending: true),
                        NSSortDescriptor(key: "id", ascending: true)
                    ]
                )
                completion(.success(try objects.map(StateLifeAreaMapper.validatedDomain)))
            } catch {
    ...
    public func create(_ area: LifeArea, completion: @escaping @Sendable (Result<LifeArea, Error>) -> Void) {
        backgroundContext.perform {
            do {
                ...
                _ = StateLifeAreaMapper.apply(normalizedArea, to: object)
