...
public final class CoreDataHabitRepository: HabitRepositoryProtocol, @unchecked Sendable {
        ...
        readContext.perform {
            ...
            do {
                let objects = try V2CoreDataRepositorySupport.fetchObjects(
                    in: self.readContext,
                    entityName: StateHabitDefinitionMapper.entityName,
                    sort: [
                        NSSortDescriptor(key: "createdAt", ascending: true),
                        NSSortDescriptor(key: "id", ascending: true)
                    ]
                )
                completion(.success(objects.map(StateHabitDefinitionMapper.toDomain)))
            } catch {
    ...
    public func fetchByID(id: UUID, completion: @escaping @Sendable (Result<HabitDefinitionRecord?, Error>) -> Void) {
        readContext.perform {
            ...
            do {
                ...
                completion(.success(object.map(StateHabitDefinitionMapper.toDomain)))
            } catch {
                completion(.failure(error))
            }
        }
    ...
    public func create(_ habit: HabitDefinitionRecord, completion: @escaping @Sendable (Result<HabitDefinitionRecord, Error>) -> Void) {
        backgroundContext.perform {
            ...
            do {
                ...
                _ = StateHabitDefinitionMapper.apply(habit, to: object)
    ...
    static func schemaValidationError(in model: NSManagedObjectModel) -> NSError? {
        ...
        guard let entity = model.entitiesByName[StateHabitDefinitionMapper.entityName] else {
            return NSError(
                domain: "CoreDataHabitRepository.Schema",
                code: 500,
                userInfo: [
                    NSLocalizedDescriptionKey: "Habit Core Data schema requirements are missing",
                    "missingRequirements": "\(StateHabitDefinitionMapper.entityName):missing_entity"
                ]
            )
        }
        ...
        return NSError(
            ...
            userInfo: [
                NSLocalizedDescriptionKey: "Habit Core Data schema requirements are missing",
                "missingRequirements": "\(StateHabitDefinitionMapper.entityName):\(missing.joined(separator: ","))"
            ]
        )
    }
}
