...
fileprivate enum SplitPersistentStore: CaseIterable {
    case cloudSync
    case localOnly
    ...
    var fileName: String {
        switch self {
        case .cloudSync:
            return PersistentStoreLocationService.cloudStoreFileName
        case .localOnly:
            return PersistentStoreLocationService.localStoreFileName
        }
    }
...
public final class PersistentStoreLocationService {
    ...
    fileprivate func activeV3StoreURLs(for store: SplitPersistentStore) -> [URL] {
        let location = resolvedV3StoreLocation()
    ...
    fileprivate func quarantineActiveV3StoreFiles(
        for store: SplitPersistentStore,
        reason: String
    ) throws -> URL? {
    ...
    fileprivate func clearActiveV3StoreFiles(for store: SplitPersistentStore) {
    ...
    private func migrateLegacySplitStoresIfNeeded(
        location: PersistentStoreLocation
    ) throws -> PersistentStoreMigrationResult {
        guard location.usesSharedAppGroupStore else {
            return PersistentStoreMigrationResult(
...
public final class PersistentStoreBootstrapService: @unchecked Sendable {
    ...
    public init(
        ...
        cloudKitContainerIdentifier: String = PersistentStoreBootstrapService.defaultCloudKitContainerIdentifier,
    ...
    public func makeV3PersistentContainer() -> NSPersistentCloudKitContainer {
        ...
        cloudDescription.configuration = SplitPersistentStore.cloudSync.configurationName
        ...
        localDescription.configuration = SplitPersistentStore.localOnly.configurationName
    ...
    public func bootstrapV3PersistentContainer() async -> PersistentStoreBootstrapResult {
        do {
            ...
            return PersistentStoreBootstrapResult(
    ...
    private func attemptCloudSyncStoreRebuild() async -> PersistentStoreBootstrapResult? {
        do {
    ...
    private func migrateSplitStoresIfNeeded() throws {
        ...
        for store in SplitPersistentStore.allCases {
    ...
    private func compatibilityReport(
        ...
        guard FileManager.default.fileExists(atPath: storeURL.path) else {
            return StoreCompatibilityReport(
                store: store,
                storeURL: storeURL,
