...
@MainActor
class SettingsPageViewController: UIViewController, CompositionRootAware {
    ...
    private func navigateToProjectManagement() {
        guard let presentationDependencyContainer else {
            assertionFailure("SettingsPageViewController requires injected CompositionRoot")
            return
        }
    ...
    private func presentCalendarChooser() {
        ...
        let host = UIHostingController(rootView: AnyView(chooser.lifeBoardTokenEnvironment(for: currentLayoutClass)))
        host.modalPresentationStyle = .pageSheet
        host.view.backgroundColor = ThemeStore.shared.currentTheme.tokens.color.bgCanvas
