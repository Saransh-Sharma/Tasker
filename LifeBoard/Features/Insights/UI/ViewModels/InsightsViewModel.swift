...
public final class InsightsViewModel: ObservableObject {
    ...
    private var pendingMutationWorkItem: CancellableDispatchWorkItem?
    ...
    public init(
        ...
        self.sessionDayKey = Self.dateKey(for: Date(), calendar: XPCalculationService.mondayCalendar())
    ...
    private func scheduleDebouncedSelectedTabRefresh() {
        ...
        pendingMutationWorkItem = CancellableDispatchWorkItem(workItem)
    ...
    private func applySystemsProjectionDelta(_ mutation: GamificationLedgerMutation) {
        ...
        let levelInfo = XPCalculationService.levelForXP(mutation.totalXP)
    ...
    private func refreshToday(version: UInt64) {
        let interval = PerformanceTrace.begin("InsightsTodayRefresh")
        let calendar = XPCalculationService.mondayCalendar()
        let now = Date()
        ...
        engine.fetchCurrentProfile { result in
            if case .success(let profile) = result {
                snapshot.update { $0.level = XPCalculationService.levelForXP(profile.xpTotal).level }
            }
            group.leave()
        }
    ...
    private func refreshWeek(version: UInt64) {
        let interval = PerformanceTrace.begin("InsightsWeekRefresh")
        let calendar = XPCalculationService.mondayCalendar()
        let today = calendar.startOfDay(for: Date())
        let weekStart = XPCalculationService.mondayStartOfWeek(for: today, calendar: calendar)
    ...
    private func refreshSystems(version: UInt64) {
        let interval = PerformanceTrace.begin("InsightsSystemsRefresh")
        let group = DispatchGroup()
        let now = Date()
    ...
    nonisolated private static func rebuiltWeeklySummaryMetrics(
        ...
        let bestDay = bestLabel(for: bars, calendar: XPCalculationService.mondayCalendar())
