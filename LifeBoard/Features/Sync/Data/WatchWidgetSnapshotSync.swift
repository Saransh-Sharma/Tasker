...
actor WatchCaptureRecoveryStore {
...
actor WatchCaptureImporter {
    ...
    func importCapture(
        ...
        var day = try await repository.fetchJournalDay(containing: dayDate)
            ?? JournalDayValue(day: dayDate)
        let now = Date()

        switch envelope.kind {
        case .mood:
            let mood = envelope.moodValue.flatMap(JournalMood.init(rawValue:)) ?? .none
...
final class WatchConnectivityCoordinator: NSObject, WCSessionDelegate, @unchecked Sendable {
    ...
    private var importer: WatchCaptureImporter?
    ...
    private let recoveryStore = WatchCaptureRecoveryStore()
    ...
    func configure(
        ...
        lock.lock()
        importer = WatchCaptureImporter(repository: repository, container: container)
    ...
    private func accept(_ envelope: WatchCaptureEnvelope, audioURL: URL? = nil) {
        ...
        Task {
            ...
            } catch WatchCaptureImporter.ImportFailure.missingAudio {
            ...
            } catch WatchCaptureImporter.ImportFailure.unsupportedEnvelope {
...
typealias WatchWidgetSnapshotSync = WatchConnectivityCoordinator
