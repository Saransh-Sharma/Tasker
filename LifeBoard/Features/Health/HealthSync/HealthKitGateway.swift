...
public enum HealthKitGatewayError: Error, Sendable, Equatable {
    case unavailable
...
/// Sample predicates are built from caller-supplied bounds, and a malformed
/// bound does not fail politely — it takes the process down.
///
/// Observed as an intermittent launch crash: `HealthSyncCoordinator`'s
/// background aggregate refresh reached
/// `+[_NSPredicateUtilities _parserableDateDescription:]`, which formats the
/// date with `%f` and faults inside `__dtoa`'s bignum path
/// (`__pow5mult_D2A` → `__mult_D2A`) with `SIGSEGV`. That path is reached for a
/// non-finite or absurdly large `timeIntervalSinceReferenceDate`, because the
/// decimal expansion it has to allocate is unbounded. Sorting it out inside
/// HealthKit is not possible, so the range is validated before a predicate is
/// ever constructed.
///
/// Callers already treat a throw as `aggregate_failed` and fall back to the
/// cached aggregate, so rejecting the range degrades one metric for one refresh
/// instead of killing the app.
enum HealthDateRangeValidator {
    /// Roughly ±1000 years around the reference date. Comfortably wider than any
    /// range the product asks for, and far below the magnitude that makes the
    /// formatter allocate unboundedly.
    static let maximumMagnitude: TimeInterval = 32_000_000_000

    static func isUsable(_ date: Date) -> Bool {
        let interval = date.timeIntervalSinceReferenceDate
        return interval.isFinite && abs(interval) <= maximumMagnitude
    }

    static func validate(start: Date, end: Date) throws {
        guard isUsable(start), isUsable(end) else {
            throw HealthKitGatewayError.invalidDateRange
        }
    }
}
...
public final class HealthKitGateway: HealthKitGatewayProtocol, @unchecked Sendable {
    ...
    public func aggregate(
        ...
        try HealthDateRangeValidator.validate(start: start, end: end)
