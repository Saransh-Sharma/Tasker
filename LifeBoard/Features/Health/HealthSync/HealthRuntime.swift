...
public final class HealthCoordinator {
    public static let shared = HealthCoordinator()
...
///
/// The policy this once owned alone now lives in `PermissionPromptState`
...
public enum HealthAuthorizationPromptState {
    public static var snoozeInterval: TimeInterval { PermissionPromptState.snoozeInterval }
    public static var maxAutoOffers: Int { PermissionPromptState.maxAutoOffers }
    ...
    public static var hasRequested: Bool {
        PermissionPromptState.hasRequested(.appleHealth)
    }

    public static var snoozedUntil: Date? {
        PermissionPromptState.snoozedUntil(.appleHealth)
    }

    public static var declineCount: Int {
        PermissionPromptState.declineCount(.appleHealth)
    }

    public static func shouldOffer(now: Date = Date()) -> Bool {
        PermissionPromptState.shouldOffer(.appleHealth, now: now)
    }

    public static func recordRequested() {
        PermissionPromptState.recordRequested(.appleHealth)
    }

    public static func recordDecline(now: Date = Date()) {
        PermissionPromptState.recordDecline(.appleHealth, now: now)
    }

    /// Test hook — clears the persisted gate.
    public static func reset() {
        PermissionPromptState.reset(.appleHealth)
    }
}
...
///
/// The prompt itself is owned by `PermissionPrimingCoordinator`, which
...
public final class HealthJustInTimeCoordinator {
    ...
    public func offerConnectIfNeeded(leadDomain: HealthDomain, trigger: String) {
        PermissionPrimingCoordinator.shared.offerIfNeeded(
            kind: .appleHealth,
            trigger: trigger,
    ...
    public func decline() {
        PermissionPrimingCoordinator.shared.decline()
    }
}
