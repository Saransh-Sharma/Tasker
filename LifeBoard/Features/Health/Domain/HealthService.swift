...
public protocol HealthReading: Sendable {
    func requestAndReadToday() async -> HealthSnapshot
}

public final class HealthService: HealthReading, @unchecked Sendable {
    ...
    public func requestAndReadToday() async -> HealthSnapshot {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
            healthStore.requestAuthorization(toShare: [], read: types) { success, error in
                if let error {
                    ...
                    continuation.resume(throwing: HealthError.authorizationUnavailable)
                }
            }
        }
    }
...
public final class HealthStore {
    public private(set) var snapshot: HealthSnapshot = .notRequested
    public private(set) var isLoading = false

    private let service: any HealthReading

    public init(service: any HealthReading = HealthService()) {
