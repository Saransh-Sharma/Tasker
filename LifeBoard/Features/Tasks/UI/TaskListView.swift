...
struct TaskListView: View {
    ...
    @State private var scrollTraceInterval: PerformanceInterval?
    ...
    private func recordScrollActivity() {
        if scrollTraceInterval == nil {
            scrollTraceInterval = PerformanceTrace.begin("HomeTaskListScrollSession")
        }
    ...
    private func finishScrollTraceIfNeeded() {
        ...
        if let scrollTraceInterval {
            PerformanceTrace.end(scrollTraceInterval)
    ...
    @ViewBuilder
    private var projectGroupedTaskContent: some View {
    ...
    private var emptyStateView: some View {
        VStack(spacing: Theme.Spacing.md) {
            DecorImage(asset: emptyStateDecorAsset, placement: .emptyState)
    ...
    private var emptyStateDecorAsset: DecorAsset {
                ...
                ForEach(Array(groups.enumerated()), id: \.element.project.id) { index, group in
                    VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
                        Text(group.project.name)
                            ...
                            .padding(.horizontal, layoutStyle.headerHorizontalPadding)
                            .padding(.top, index == 0 ? 0 : Theme.Spacing.sm)
