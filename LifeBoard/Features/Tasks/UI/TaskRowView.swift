...
struct TaskRowView: View, Equatable {
    ...
    private var resolvedIconTint: Color {
        HexColor.color(accentHex, fallback: Color.lifeboard.accentPrimary)
    }
    ...
    private var rowContent: some View {
        HStack(spacing: 0) {
            ...
            .padding(.trailing, isSunriseSearchCard ? Theme.Spacing.md : Theme.Spacing.md)
            .padding(.leading, isSunriseSearchCard ? Theme.Spacing.sm : Theme.Spacing.xs)
    ...
    private var taskRowHighlightShape: AnyShape {
        ...
        case .card, .sunriseSearchCard:
            return AnyShape(RoundedRectangle(cornerRadius: Theme.CornerRadius.md))
        case .flatHomeList:
    ...
    private var rowInnerSpacing: CGFloat {
        if isSunriseSearchCard { return Theme.Spacing.sm }
        return isPad ? Theme.Spacing.sm : Theme.Spacing.xs
    }
