...
struct TaskSectionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isExpanded {
                taskList
                    .padding(.top, layoutStyle == .inset ? Theme.Spacing.xs : 0)
...
struct HomeListSectionView: View {
    ...
    private var rowsContent: some View {
        ...
        .padding(.top, section.showsHeader ? (layoutStyle == .inset ? Theme.Spacing.xs : 0) : 0)
        .transition(.asymmetric(
            insertion: .opacity.combined(with: .move(edge: .top)).combined(with: .scale(scale: 0.98, anchor: .top)),
            removal: .opacity
        ))
    ...
    private var accentColor: Color {
        ...
        return HexColor.color(accentHex, fallback: Color.lifeboard.accentPrimary)
    }
