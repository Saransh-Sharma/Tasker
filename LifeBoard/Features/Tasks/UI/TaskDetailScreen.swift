...
private enum TaskDetailSection: String, Hashable, CaseIterable {
    case plan
    case notes
    case steps
    case organize
...
struct TaskDetailScreen: View {
    ...
    private var primaryActionCluster: some View {
        HStack(spacing: spacing.s8) {
            ...
            .buttonStyle(DetailCapsuleButtonStyle(tone: viewModel.isComplete ? .quiet : .success))
        ...
        .lifeboardChromeSurface(cornerRadius: Theme.CornerRadius.card, accentColor: Color.lifeboard.accentPrimary)
    }
                ...
                HStack(spacing: spacing.s8) {
                    if viewModel.dueDate != nil {
                        Button("Clear due date") { viewModel.setDueDate(nil) }
                            .buttonStyle(TextButtonStyle(tone: .warning))
                    }
    ...
    private var notesContent: some View {
        VStack(alignment: .leading, spacing: spacing.s12) {
            ...
            .buttonStyle(DetailCapsuleButtonStyle(tone: .accent))
        }
    }
        ...
        disclosureCard(.steps, title: "Steps", systemImage: "list.bullet", summary: viewModel.stepsSummary, accessibilityIdentifier: "taskDetail.disclosure.steps") {
            VStack(alignment: .leading, spacing: spacing.s8) {
                ...
                ForEach(viewModel.childSteps, id: \.id) { step in
                    ...
                    .padding(spacing.s12)
                    .lifeboardDenseSurface(cornerRadius: Theme.CornerRadius.md, fillColor: Color.lifeboard.surfaceSecondary.opacity(0.72))
    ...
    private var taskFitHintRow: some View {
        ...
        .padding(spacing.s12)
        .lifeboardDenseSurface(cornerRadius: Theme.CornerRadius.md, fillColor: style.fill, strokeColor: style.stroke)
    ...
    private func projectMotivationCard(_ motivation: ProjectWeeklyMotivation) -> some View {
        ...
        .padding(spacing.s12)
        .lifeboardDenseSurface(cornerRadius: Theme.CornerRadius.md, fillColor: Color.lifeboard.accentWash.opacity(0.72))
    }
    ...
    private func expand(_ section: TaskDetailSection) {
        withAnimation(LifeBoardAnimation.stateChange) {
            _ = expandedSections.insert(section)
        }
    }

    private func toggle(_ section: TaskDetailSection) {
    ...
    private static func defaultExpandedSections(for task: TaskDefinition) -> Set<TaskDetailSection> {
...
private struct TaskDetailHeroMetricRow: View {
    ...
    private func metric(title: String, value: String, detail: String, tone: HeroMetricTone) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(.lifeboard(.meta))
                .foregroundStyle(Color.lifeboard.textTertiary)
