...
struct DaySwipeHandleInteractionState: Equatable {
...
struct DaySwipeOverlay: View {
    ...
    let onHandleDragChanged: (DaySwipeSide, CGSize, CGPoint, CGSize) -> Void
    let onHandleDragEnded: (DaySwipeSide, CGSize, CGSize, CGPoint, CGSize) -> Void

    @Binding var leadingData: DaySwipeData
    @Binding var trailingData: DaySwipeData
    @Binding var topSide: DaySwipeSide
    @State private var lastDraggedHandleSide: DaySwipeSide?
    @State private var lastHandleDragEndedAt: Date?
    @State private var handleInteractionState = DaySwipeHandleInteractionState()
    ...
    private func sideLayer(_ side: DaySwipeSide, size: CGSize) -> some View {
        let data = swipeData(for: side, size: size)
        let isInteractive = isEnabled && isChromeVisible
        let presentation = DaySwipeChromePresentation.value(
            for: side,
        ...
        return ZStack(alignment: .topLeading) {
            ...
            Button {
                ...
                Label(side.accessibilityLabel, systemImage: side.systemImage)
                    .labelStyle(.iconOnly)
                    .font(.system(size: 14 * DaySwipeData.buttonVisualScale, weight: .bold))
                    .symbolRenderingMode(.hierarchical)
                    .foregroundStyle(Color.lifeboard.accentOnPrimary)
                    .frame(
                        width: DaySwipeData.buttonVisualRadius * 2,
                        height: DaySwipeData.buttonVisualRadius * 2
                    )
                    .background {
                        Circle()
                    ...
                    .frame(
                        width: DaySwipeData.buttonRadius * 2,
                        height: DaySwipeData.buttonRadius * 2
                    )
                    .contentShape(Circle())
            }
            .buttonStyle(.plain)
    ...
    private func commit(_ side: DaySwipeSide, size: CGSize) {
        guard isEnabled, isChromeVisible else { return }
        let interval = PerformanceTrace.begin("SunriseDaySwipeCommit")
    ...
    private func shouldCommitTap(for side: DaySwipeSide) -> Bool {
    ...
    private func reset(_ side: DaySwipeSide, size: CGSize) {
    ...
    private func swipeData(for side: DaySwipeSide, size: CGSize) -> DaySwipeData {
    ...
    private func setSwipeData(_ data: DaySwipeData) {
        switch data.side {
        case .leading:
            leadingData = data
        case .trailing:
...
struct DaySwipeGestureSurface: UIViewRepresentable {
    let isEnabled: Bool
    let containerSize: CGSize
    let restingCenterY: CGFloat
    let resolver: HomeDaySwipeResolver
    let onInteractionStarted: () -> Void
    let onChanged: (DaySwipeSide, CGSize, CGPoint) -> Void
    let onEnded: (DaySwipeSide, CGSize, CGSize, CGPoint) -> Void
    let onCancelled: (DaySwipeSide) -> Void
    ...
    final class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        private func isHandleLocation(_ location: CGPoint, in size: CGSize) -> Bool {
            let hitRadius = DaySwipeData.buttonRadius + 12
            return DaySwipeSide.allCases.contains { side in
                let center = DaySwipeData(
                    side: side,
                    containerSize: size,
