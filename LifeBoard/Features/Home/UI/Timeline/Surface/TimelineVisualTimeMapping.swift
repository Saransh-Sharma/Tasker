...
// The mapping that compresses empty hours so a sparse day still reads as
// a day. 409 lines in one extension, baselined at that size.

extension TimelineCanvasLayoutPlan {
    static func maxVisualColumns(for layoutClass: LayoutClass) -> Int {
        switch layoutClass {
        case .phone:
            return 3
