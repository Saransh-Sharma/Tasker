...
enum TaskListWidgetCalendarProjection {
    ...
    static func weekDays(
        from sourceEvents: [CalendarEventSnapshot],
        anchorDate: Date,
        weekStartsOn: Weekday,
        calendar: Calendar
    ...
    static func nextMeeting(
        from summary: NextMeetingSummary
    ...
    static func calendarEvent(
        from event: CalendarEventSnapshot
        ...
        coordinator.getTasks.searchTasks(query: "", in: .all) { [weak self] result in
            ...
            case .success(let tasks):
                ...
                Task { @MainActor [weak self] in
                    ...
                    let workspacePreferences = WorkspacePreferencesStore.shared.load()
                    let now = Date()
