...
extension HomeViewModel {
    ...
    func performHabitMutation(
        ...
        pendingHabitMutationIntervals[key] = PerformanceTrace.begin("HomeUserMutation")
        if Calendar.current.isDate(date, inSameDayAs: selectedDate) {
            pendingHabitMutationSnapshots[key] = captureHabitMutationSnapshot()

            let applyInterval = PerformanceTrace.begin("HomeHabitOptimisticApply")
            let didApplyOptimisticUpdate = applyOptimisticHabitMutation(
                row,
                request: request,
                on: date
            )
            PerformanceTrace.end(applyInterval)
            ...
            PerformanceTrace.event("HomeUserMutationOptimisticApplied")
        }
    ...
    func handleHabitMutationResult(
        ...
        case .success:
            PerformanceTrace.event("HomeUserMutationPersistenceComplete")
            pendingHabitMutationSnapshots.removeValue(forKey: key)
            pendingHabitMutationKeys.remove(key)
            habitMutationErrorMessage = nil
    ...
    func applyOptimisticHabitMutation(
        ...
        PerformanceTrace.event("HomeUserMutationRecomputedRows", value: patch.affectedRowCount)
        PerformanceTrace.event("HomeUserMutationRecomputedSections", value: patch.affectedSectionCount)
        return true
    }
