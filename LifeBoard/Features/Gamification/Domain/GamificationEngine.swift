...
public struct XPCelebrationPayload: Equatable, Sendable {
    ...
    public let crossedMilestone: XPCalculationService.Milestone?
...
public final class GamificationService: @unchecked Sendable {
    ...
    public func recordCompensationEvent(
        ...
        let delta = XPCalculationService.baseXP(for: context.category)
        guard delta < 0 else {
    ...
    public func writeWidgetSnapshot() {
        fetchTodayXP { [weak self] xpResult in
            ...
            self.fetchCurrentProfile { profileResult in
                ...
                let levelInfo = XPCalculationService.levelForXP(profile.xpTotal)
                let milestone = XPCalculationService.nextMilestone(for: profile.xpTotal)
                let calendar = XPCalculationService.mondayCalendar()
                let now = Date()
                let today = calendar.startOfDay(for: now)
                let tomorrow = calendar.date(byAdding: .day, value: 1, to: today) ?? now
                let weekStart = XPCalculationService.mondayStartOfWeek(for: now, calendar: calendar)
                let weekEndDate = calendar.date(byAdding: .day, value: 6, to: weekStart) ?? weekStart

                var milestoneProgress: Double = 0
                if let ms = milestone {
                    let prev = XPCalculationService.milestones
            ...
            case .success(let existingProfile):
                ...
                self.repository.fetchXPEvents { eventsResult in
                    ...
                    case .success(let allEvents):
                        ...
                        let levelInfo = XPCalculationService.levelForXP(xpTotal)

                        // Recompute daily aggregates
                        let grouped = Dictionary(grouping: unique) {
                            XPCalculationService.periodKey(for: $0.createdAt)
                        }
    ...
    private func calculateAndRecord(
        ...
        let periodKey = XPCalculationService.periodKey(for: context.completedAt)
        ...
        repository.fetchDailyAggregate(dateKey: periodKey) { [weak self] aggResult in
            ...
            case .success(let existingAgg):
                ...
                if context.category == .focus {
                    base = XPCalculationService.focusSessionXP(durationSeconds: context.focusDurationSeconds ?? 0)
                    bonus = 0
                    weight = 1.0
                } else if XPCalculationService.isHabitCategory(context.category) {
                    base = XPCalculationService.baseXP(for: context.category)
                    bonus = 0
                    weight = 1.0
                } else {
                    base = XPCalculationService.baseXP(for: context.category)
                    bonus = (context.category == .complete && XPCalculationService.isOnTimeCompletion(
                        dueDate: context.dueDate,
                        completedAt: context.completedAt
                    )) ? XPCalculationService.onTimeBonusXP() : 0

                    weight = XPCalculationService.qualityWeight(
                ...
                let finalXP = XPCalculationService.calculateFinalXP(
                    base: base,
                    bonus: bonus,
                    qualityWeight: weight
                )

                let shouldRecordZeroXPEvent = XPCalculationService.isHabitCategory(context.category)
    ...
    private func finishRecordedEvent(
        ...
        evaluateAchievements(triggerEvent: event) { achieveResult in
            ...
            let levelInfo = XPCalculationService.levelForXP(profile.xpTotal)
            let milestone = XPCalculationService.milestoneCrossed(
                ...
                case .success:
                    self.fetchCurrentState(for: context.completedAt) { stateResult in
                        ...
                        case .success(let (profile, dailyXP)):
                            ...
                            let previousLevel = XPCalculationService.levelForXP(previousXP).level
                            let result = XPEventResult(
                                awardedXP: delta,
                                totalXP: profile.xpTotal,
                                level: levelInfo.level,
            ...
            case .success:
                self.fetchCurrentState { stateResult in
                    ...
                    case .success(let (profile, dailyXP)):
                        ...
                        let previousLevel = XPCalculationService.levelForXP(previousXP).level
                        let crossedMilestone = XPCalculationService.milestoneCrossed(
    ...
    private func pendingUnlocks(
        ...
        let habitSuccessEvents = unique.filter {
            ...
            return XPCalculationService.isHabitCategory(category)
    ...
    private func emitLedgerMutation(
        ...
        let payload = GamificationLedgerMutation(
            ...
            didChange: didChange,
            dateKey: XPCalculationService.periodKey(for: context.completedAt),
            occurredAt: Date(),
    ...
    private func idempotencyKey(for context: XPEventContext) -> String {
        XPCalculationService.idempotencyKey(
            ...
            toDay: context.toDay,
            periodKey: XPCalculationService.periodKey(for: context.completedAt)
        )
    }
    ...
    private func habitSuccessCompensationPrefix(for context: XPEventContext) -> String? {
        ...
        case .habitPositiveComplete:
            return "habit_positive_complete_undo:\(identifier.uuidString):\(XPCalculationService.periodKey(for: context.completedAt))"
        case .habitNegativeSuccess:
            return "habit_negative_success_undo:\(identifier.uuidString):\(XPCalculationService.periodKey(for: context.completedAt))"
        default:
            return nil
        }
    }
    ...
    private func fetchCurrentState(completion: @escaping @Sendable (Result<(GamificationSnapshot, Int), Error>) -> Void) {
        fetchCurrentState(forDateKey: XPCalculationService.periodKey(), completion: completion)
    }
    ...
    private func fetchCurrentState(
        ...
        fetchCurrentState(forDateKey: XPCalculationService.periodKey(for: date), completion: completion)
    }
