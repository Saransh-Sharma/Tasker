...
public enum MoodTrendProjector {
    public static func project(
        _ checkIns: [MoodEnergyCheckInValue],
        ...
        let valued = checkIns.compactMap { checkIn -> (MoodEnergyCheckInValue, Double)? in
            guard let index = JournalMood.dialOrder.firstIndex(of: checkIn.mood) else { return nil }
...
public enum TrackCorrectionPayload: Codable, Hashable, Sendable {
    ...
    case mood(MoodEnergyCheckInValue)
    case tracker(TrackerEntryValue)
    case medication(MedicationEventValue)
    case fasting(FastingSessionValue)
...
public struct EvidenceAuthorizationPolicy: Sendable {
    ...
    public func allowedDestinations(domain: String, sensitivity: DataSensitivity) -> Set<Destination> {
        switch domain {
    ...
    public func redaction(sensitivity: DataSensitivity, destination: Destination) -> RedactionPolicy {
...
public protocol LifeEventProjectionRepository: Sendable {
    func authorizedEvents(for destination: Destination, journalConsentGranted: Bool) -> [NormalizedLifeEvent]
}
...
public struct SnapshotLifeEventProjectionRepository: LifeEventProjectionRepository {
    ...
    public func authorizedEvents(for destination: Destination, journalConsentGranted: Bool) -> [NormalizedLifeEvent] {
        authorizedEvents(
            for: destination,
...
public struct TrackTodaySnapshot: Codable, Equatable, Sendable {
    public var unresolvedMedicationEvents: [MedicationEventValue]
...
public protocol HabitGradeService: Sendable {
    func evaluate(
        habitID: UUID,
...
public struct RoutineHomeContextCandidateSource: HomeContextCandidateSource {
    ...
    public init(
        ...
        daypart: @escaping @Sendable (Date) -> ResolvedDaypart = { DaypartResolver.resolve(at: $0) }
    ) {
        self.repository = repository
        self.daypart = daypart
    }
