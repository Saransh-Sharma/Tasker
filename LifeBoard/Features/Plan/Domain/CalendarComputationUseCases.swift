...
public struct BuildCalendarBusyBlocksUseCase: Sendable {
    ...
    public func execute(
        ...
        let intervals = events
            ...
            .map { event -> CalendarBusyBlock in
                let clampedStart = max(referenceStart, event.startDate)
                let clampedEnd = min(referenceEnd, event.endDate)
                return CalendarBusyBlock(startDate: clampedStart, endDate: clampedEnd)
            }
            .filter { $0.duration > 0 }
            .sorted { lhs, rhs in
                if lhs.startDate != rhs.startDate {
...
public struct BuildCalendarWeekAgendaUseCase {
    ...
    public func execute(
        ...
        for offset in 0..<7 {
            ...
            days.append(CalendarDayAgenda(date: start, events: dayEvents))
        }
        return days
    }
}
