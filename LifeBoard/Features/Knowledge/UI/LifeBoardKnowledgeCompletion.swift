...
public enum NoteMutation: Hashable, Sendable {
    ...
    case insertBlock(KnowledgeBlockValue, at: Int)
    case deleteBlock(KnowledgeBlockValue, at: Int)
    case moveBlock(id: UUID, from: Int, to: Int)
    case changeBlockKind(id: UUID, from: KnowledgeBlockKind, to: KnowledgeBlockKind)
    ...
    case replaceNote(before: KnowledgeNoteValue, after: KnowledgeNoteValue)
...
public struct KnowledgeNoteDocument: @unchecked Sendable {
    ...
    public init(note: KnowledgeNoteValue, traitCollection: UITraitCollection = .current) {
    ...
    public func blocks(noteID: UUID, previous: [KnowledgeBlockValue]) -> [KnowledgeBlockValue] {
        let source = attributedString.string as NSString
        let byID = Dictionary(uniqueKeysWithValues: previous.map { ($0.id, $0) })
        var values: [KnowledgeBlockValue] = []
        ...
        while cursor < NSMaxRange(paragraphs) || (source.length == 0 && values.isEmpty) {
            ...
            var value = byID[id] ?? KnowledgeBlockValue(id: id, noteID: noteID)
            let raw = attributes[Self.blockKindKey] as? String
            value.kind = raw.flatMap(KnowledgeBlockKind.init(rawValue:)) ?? value.kind
    ...
    public static func split(
        block: KnowledgeBlockValue,
        atUTF16Offset offset: Int
    ) -> (leading: KnowledgeBlockValue, trailing: KnowledgeBlockValue) {
    ...
    public static func merge(
        leading: KnowledgeBlockValue,
        trailing: KnowledgeBlockValue
    ) -> KnowledgeBlockValue {
        var merged = leading
    ...
    private static func renderedText(for block: KnowledgeBlockValue) -> String {
        switch block.kind {
    ...
    private static func plainText(fromRenderedText text: String, kind: KnowledgeBlockKind) -> String {
        switch kind {
...
public enum KnowledgeEditorCommand: Hashable, Sendable {
    ...
    case highlight
    case inlineCode
    case block(KnowledgeBlockKind)
    case insertBlock(KnowledgeBlockKind)
    case indent(Int)
...
public struct UnifiedNoteEditor: UIViewRepresentable {
    @Binding private var note: KnowledgeNoteValue
    ...
    public init(
        note: Binding<KnowledgeNoteValue>,
    ...
    public final class Coordinator: NSObject, UITextViewDelegate {
        ...
        var lastRenderedNote: KnowledgeNoteValue?
        var lastCommandID: UUID?
        var isApplyingTextChange = false

        init(parent: UnifiedNoteEditor) {
            self.parent = parent
        }
        ...
        func apply(_ command: KnowledgeEditorCommand, to textView: UITextView) {
            ...
            case let .block(kind):
                ...
                let block = KnowledgeBlockValue(noteID: parent.note.id, kind: kind)
        ...
        private func normalizeBlockAttributes(in textView: UITextView, blocks: [KnowledgeBlockValue]) {
            let source = textView.text as NSString
            var cursor = 0
...
public final class NoteEditorSession {
    public let id = UUID()
    public private(set) var note: KnowledgeNoteValue
    ...
    public func recoverDraftIfNewer() async -> KnowledgeNoteValue? {
        guard let draft = try? await repository.fetchKnowledgeDraft(noteID: note.id),
              ...
              let recovered = try? JSONDecoder().decode(KnowledgeNoteValue.self, from: draft.snapshot) else {
            return nil
        }
        return recovered
    }

    public func restoreRecoveredDraft(_ recovered: KnowledgeNoteValue) {
        note = recovered
        scheduleAutosave()
    }
...
public struct KnowledgeUnlockedNoteSession: Hashable, Sendable {
    public var note: KnowledgeNoteValue
    public var attachments: [KnowledgeAttachmentValue]
...
public protocol KnowledgeSecureNoteService: Sendable {
    ...
    func unlock(_ envelope: KnowledgeSecureEnvelope, reason: String) async throws
        -> (KnowledgeNoteValue, [KnowledgeAttachmentValue], [KnowledgeLinkValue])
    func reseal(
        note: KnowledgeNoteValue,
        attachments: [KnowledgeAttachmentValue],
        links: [KnowledgeLinkValue],
...
public actor DefaultKnowledgeSecureNoteService: KnowledgeSecureNoteService {
    private struct CanonicalPayload: Codable {
        var note: KnowledgeNoteValue
        var attachmentMetadata: [KnowledgeAttachmentValue]
        var links: [KnowledgeLinkValue]?
    }
    ...
    private func decryptWithoutAuthentication(
        _ envelope: KnowledgeSecureEnvelope
    ) throws -> (KnowledgeNoteValue, [KnowledgeAttachmentValue], [KnowledgeLinkValue]) {
...
public struct NoteAIProposal: Identifiable, Hashable, Sendable {
    ...
    public func isStale(for note: KnowledgeNoteValue) -> Bool {
...
public struct ModelsNoteAIProposalService: NoteAIProposalService {
...
public enum KnowledgeMarkdownCodec {
    public static func render(_ note: KnowledgeNoteValue, includeTitle: Bool = true) -> String {
        var lines: [String] = []
        if includeTitle {
    ...
    public static func parse(
        ...
        var blocks: [KnowledgeBlockValue] = []
        var codeLines: [String] = []
        var isInCodeFence = false

        func append(_ kind: KnowledgeBlockKind, _ text: String, checked: Bool = false) {
            blocks.append(
                .init(
                    noteID: noteID,
                    kind: kind,
...
public protocol KnowledgeAttachmentPipeline: Sendable {
    func ingest(
        ...
        sourceKind: String,
        noteID: UUID
    ) async throws -> KnowledgeAttachmentValue
...
public actor LocalKnowledgeAttachmentPipeline: KnowledgeAttachmentPipeline {
    ...
    public func ingest(
        ...
        var attachment = KnowledgeAttachmentValue(
            noteID: noteID,
