...
public struct ScannedDraft: Identifiable, Hashable, Sendable {
    public let id: UUID
...
public enum DocumentScanError: LocalizedError, Sendable {
    case unavailable
    case noReadableText
...
public struct DocumentScannerView: UIViewControllerRepresentable {
    ...
    private let completion: @MainActor (Result<ScannedDraft, Error>) -> Void
    private let cancellation: @MainActor () -> Void

    public init(
        completion: @escaping @MainActor (Result<ScannedDraft, Error>) -> Void,
    ...
    public final class Coordinator: NSObject, @preconcurrency VNDocumentCameraViewControllerDelegate {
            ...
            Task {
                do {
                    ...
                    guard trimmed.isEmpty == false else {
                        completion(.failure(DocumentScanError.noReadableText))
                        return
                    }
        ...
        private nonisolated static func recognize(_ pages: [Data]) async throws -> String {
            try await Task.detached(priority: .userInitiated) {
                ...
                guard pageTexts.isEmpty == false else { throw DocumentScanError.noReadableText }
...
public struct BarcodeScannerView: View {
    ...
    public var body: some View {
        Color.clear.task { completion(.failure(BarcodeScanError.unavailable)) }
    }
}
#else
@available(iOS 16.0, *)
public struct BarcodeScannerView: UIViewControllerRepresentable {
    ...
    public func updateUIViewController(_ controller: DataScannerViewController, context: Context) {
        ...
        catch { completion(.failure(BarcodeScanError.unavailable)) }
    }
...
public enum BarcodeScannerCapability {
    @MainActor
