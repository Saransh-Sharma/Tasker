...
public actor JournalProjectionInvalidationService {
    public enum Event: Sendable {
        case projectionsInvalidated
        case reflectionsInvalidated(Set<UUID>)
    }

    public static let shared = JournalProjectionInvalidationService()
...
public actor JournalDerivedPipelineCoordinator {
    ...
    public func processCommitted(_ snapshot: JournalEntrySnapshot) async throws {
        let interval = PerformanceOperation.journalDerivedRebuild.begin()
        defer { PerformanceOperation.journalDerivedRebuild.end(interval) }
