...
public struct FocusZone: View {
    ...
    private var focusHeaderLabel: some View {
        HStack(spacing: spacing.s4) {
            DecorImage(asset: hasTaskRows ? .thinkingCup : .cloud, size: 28, opacity: hasTaskRows ? 0.92 : 0.64)
    ...
    private var emptyState: some View {
        VStack(spacing: spacing.s4) {
            DecorImage(asset: .thinkingCup, size: 34, opacity: 0.72)
                .opacity(0.72)
        ...
        if presentation.secondarySegments.isEmpty == false {
            (
                presentation.secondarySegments.enumerated().reduce(Text("")) { partialResult, entry in
                    ...
                    let separator = Text(FocusZoneSecondaryFormatting.separator)
                        .font(.lifeboard(.caption1))
