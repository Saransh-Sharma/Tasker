...
final class TaskSemanticRetrievalService: Sendable {
    ...
    private let embeddingEngine: TaskEmbeddingService
    private let indexStore: TaskSemanticIndexStore
    private let activationState = TaskSemanticActivationState()

    /// Initializes a new instance.
    init(
        embeddingEngine: TaskEmbeddingService = TaskEmbeddingService(),
    ...
    func searchDetailed(
        ...
        let hits = candidates.map { item in
            TaskSemanticHit(
                taskID: item.taskID,
                score: TaskEmbeddingService.cosineSimilarity(queryVector, item.vector),
                text: item.text
            )
        }
