...
final class AssistantIdentityModel: ObservableObject {
    ...
    init(workspacePreferencesStore: WorkspacePreferencesStore = .shared) {
        ...
        self.cancellable = NotificationCenter.default.publisher(for: WorkspacePreferencesStore.didChangeNotification)
...
enum AssistantIdentityText {
    ...
    static func currentSnapshot() -> AssistantIdentitySnapshot {
        AssistantIdentitySnapshot(mascotID: WorkspacePreferencesStore.shared.load().chiefOfStaffMascotID)
    }
...
struct EvaMascotView: View {
    ...
    @State private var selectedMascotID = WorkspacePreferencesStore.shared.load().chiefOfStaffMascotID
    ...
    var body: some View {
        mascotBody
            ...
            .onReceive(NotificationCenter.default.publisher(for: WorkspacePreferencesStore.didChangeNotification)) { notification in
                guard mascotID == nil else { return }
                if let preferences = notification.object as? WorkspacePreferences {
                    selectedMascotID = preferences.chiefOfStaffMascotID
                } else {
                    selectedMascotID = WorkspacePreferencesStore.shared.load().chiefOfStaffMascotID
                }
            }
    }
