                                    ...
                                    VStack(alignment: .leading, spacing: 6) {
                                        ...
                                        HStack(spacing: spacing.s8) {
                                            if let projectName = outcome.projectName {
                                                StatusPill(text: projectName, systemImage: "folder", tone: .quiet)
                                            }
                                            StatusPill(
...
private struct WeeklyPlannerTriageCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        StatusPill(text: model.task.priority.displayName, systemImage: "flag.fill", tone: model.task.priority.isHighPriority ? .warning : .quiet)
                    }
                }
            ...
            HStack(spacing: 8) {
                StatusPill(text: model.currentPlacementText, systemImage: "arrow.triangle.branch", tone: .quiet)
                if let outcomeTitle = model.outcomeTitle, outcomeTitle.isEmpty == false {
                    StatusPill(text: outcomeTitle, systemImage: "scope", tone: .accent)
                }
            }
        ...
        NavigationStack {
            WeeklyRitualScaffold(
                ...
                WeeklySectionCard(
                    ...
                    HStack(spacing: 12) {
                        DecorImage(asset: .decisionSign, size: 44, opacity: 0.9)
