...
public struct LifeBoardComposerReceipt: Equatable, Sendable {
...
public typealias LifeBoardComposerPhase = AsyncActionPhase<LifeBoardComposerReceipt>
...
/// `LifeBoardTrackFoundationRootView.categoryContent`; this is the same rule one
/// level down.
@MainActor
public struct LifeBoardComposerScaffold<Content: View, Commit: View>: View {
    ...
    fileprivate var confirmation: LifeBoardComposerConfirmation?
    ...
    public var body: some View {
        LifeBoardPresentationScaffold(mode: .editor) {
            NavigationStack {
    ...
    private var scroll: some View {
        ScrollView {
            ...
            .padding(.bottom, 24)
            .modifier(LifeBoardComposerPrivacy(isSensitive: isPrivacySensitive))
            ...
            .modifier(LifeBoardComposerIdentity(identifier: identifier))
        }
...
public extension LifeBoardComposerScaffold where Commit == EmptyView {
    ...
    init(
        ...
        self.confirmation = LifeBoardComposerConfirmation(
            title: confirmTitle,
...
struct LifeBoardComposerConfirmation {
    let title: String
    let isEnabled: Bool
    let action: () -> Void
}
...
///
/// `LifeBoardComposerScaffold` owns a `NavigationStack` and sheet presentation,
...
@MainActor
public struct LifeBoardComposerPage<Content: View, Commit: View>: View {
    private let subtitle: String?
    private let isPrivacySensitive: Bool
    private let identifier: String?
    private let content: Content
    ...
    public var body: some View {
        ...
        .scrollIndicators(.hidden)
        .background { LifeBoardComposerCanvas() }
        .safeAreaInset(edge: .bottom, spacing: 0) { commit }
    }
}
...
private struct LifeBoardComposerCanvas: View {
    var body: some View {
        LifeBoardGrainedCanvas(intensity: 0.42)
    }
}
...
/// `LifeBoardKnowledgeNoteEditor` from under to over the 500 ms type-check
...
public struct LifeBoardGrainedCanvas: View {
...
private struct LifeBoardComposerPrivacy: ViewModifier {
...
private struct LifeBoardComposerIdentity: ViewModifier {
...
public struct LifeBoardComposerSection<Content: View>: View {
    private let header: String?
    private let detail: String?
    private let footer: String?
    private let depth: LifeBoardClayDepth
    ...
    public init(
        ...
        detail: String? = nil,
        footer: String? = nil,
        depth: LifeBoardClayDepth = .resting,
    ...
    public var body: some View {
        ...
        .accessibilityElement(children: .contain)
        .modifier(LifeBoardComposerSectionLabel(header: header))
    }
}

private struct LifeBoardComposerSectionLabel: ViewModifier {
    let header: String?
...
public struct LifeBoardComposerRow<Trailing: View>: View {
...
public struct LifeBoardComposerCommitBar: View {
    private let title: String
