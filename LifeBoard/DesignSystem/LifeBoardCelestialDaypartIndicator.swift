...
public enum LifeBoardDaypartProgress {
    ...
    /// Start hour and length, in hours, matching `LifeBoardCelestialPhase.resolve`.
    static func bounds(of phase: LifeBoardCelestialPhase) -> (start: Double, length: Double) {
        switch phase {
        case .dawn: (5, 3)
        case .morning: (8, 4)
        case .midday: (12, 5)
    ...
    public static func isDaylight(at date: Date, calendar: Calendar = .current) -> Bool {
        LifeBoardCelestialPhase.resolve(at: date, calendar: calendar) != .night
    }
}
...
public struct LifeBoardCelestialDaypartIndicator: View {
    ...
    private var phase: LifeBoardCelestialPhase {
        LifeBoardCelestialPhase.resolve(at: date, calendar: calendar)
    }

    private var progress: Double {
        LifeBoardDaypartProgress.arcProgress(at: date, calendar: calendar)
    }
    ...
    private func celestialBody(side: CGFloat) -> some View {
        let point = LifeBoardCelestialArc.point(
            forProgress: progress,
            in: CGRect(x: 0, y: 0, width: side, height: side / 2)
        )
        return Image(decorative: LifeBoardAtmosphereDescriptor.descriptor(for: phase).celestialAsset)
            .resizable()
            .scaledToFit()
    ...
    private var accessibilityDescription: String {
        ...
        let period = LifeBoardDaypartProgress.isDaylight(at: date, calendar: calendar)
            ? "daylight"
            : "night"
...
public struct LifeBoardCelestialArc: Shape {
