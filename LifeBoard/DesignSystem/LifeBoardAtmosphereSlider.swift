...
///
/// Kept apart from the view for the same reason `LifeBoardArcDialGeometry` is:
...
public enum LifeBoardAtmosphereSliderGeometry {
...
public struct LifeBoardAtmosphereSlider: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            // A slider cannot answer large type the way `LifeBoardLensPicker`
    ...
    private func dragGesture(innerWidth: CGFloat) -> some Gesture {
        ...
        DragGesture(minimumDistance: 0)
            .onChanged { value in
                ...
                let index = LifeBoardAtmosphereSliderGeometry.index(
    ...
    private func dayGradient(innerWidth: CGFloat) -> some View {
        let locations = LifeBoardAtmosphereSliderGeometry.dayGradientLocations(innerWidth: innerWidth)
        ...
        return Capsule()
            ...
            .frame(width: LifeBoardAtmosphereSliderGeometry.dayLaneWidth(innerWidth: innerWidth))
            .offset(x: laneOffset(
                LifeBoardAtmosphereSliderGeometry.dayLaneOrigin(),
                width: LifeBoardAtmosphereSliderGeometry.dayLaneWidth(innerWidth: innerWidth),
                innerWidth: innerWidth
            ))
    }
    ...
    /// five.
    private func artPalette(_ daypart: ResolvedDaypart) -> LifeBoardDaypartPalette {
        colorScheme == .dark
            ? LifeBoardDaypartTokens.darkPalette(for: daypart)
    ...
    private func daypartTint(_ daypart: ResolvedDaypart) -> Color {
        ...
        // part of the track.
        let role: LifeBoardDaypartColorRole = daypart == .night ? .canvas : .celestialPrimary
    ...
    private func autoLane(innerWidth: CGFloat) -> some View {
        Capsule()
            ...
            .frame(width: LifeBoardAtmosphereSliderGeometry.autoLaneWidth)
            .offset(x: laneOffset(
                0,
                width: LifeBoardAtmosphereSliderGeometry.autoLaneWidth,
                innerWidth: innerWidth
            ))
    }

    private func separator(innerWidth: CGFloat) -> some View {
        let origin = LifeBoardAtmosphereSliderGeometry.autoLaneWidth
            + LifeBoardAtmosphereSliderGeometry.separatorWidth / 2
        return Rectangle()
    ...
    private func stopGlyphs(innerWidth: CGFloat) -> some View {
        ForEach(Array(Self.stops.enumerated()), id: \.element) { index, stop in
            Image(systemName: stop.systemImage)
                ...
                .offset(
                    x: LifeBoardAtmosphereSliderGeometry.x(
    ...
    private func knob(innerWidth: CGFloat) -> some View {
        let x = LifeBoardAtmosphereSliderGeometry.x(
