...
extension SunriseAppShellView {
    func todayTimelineSurface(taskListBottomInset: CGFloat) -> some View {
        VStack(spacing: 0) {
            ...
            ZStack {
                ScrollView {
                    timelineScrollContent(taskListBottomInset: taskListBottomInset)
                }
    ...
    func timelineScrollContent(taskListBottomInset: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: spacing.s12) {
            timelineQuietTrackingUITestMarkerSection
            timelinePassiveTrackingRailSection
            timelineNeedsReplanTraySection
            AnyView(timelineRescueTail)
            timelineMainSurfaceSection
            timelineDailyReflectionSection
            timelineFooterModulesSection
            timelineGuidanceSection
            timelinePersistentReplanEntrySection
            AnyView(timelineBottomContentSpacer(taskListBottomInset: taskListBottomInset))
        }
        .padding(.top, spacing.s8)
        .contentShape(Rectangle())
        .lifeboardScrollOptimizedRendering()
        .background {
            timelineDaySwipeGestureSurface
        }
    }

    var timelineQuietTrackingUITestMarkerSection: AnyView {
        guard showsFullTimelineQuietTrackingUITestMarker else { return AnyView(EmptyView()) }
        return AnyView(
    ...
    var timelinePassiveTrackingRailSection: AnyView {
        guard habitsSnapshot.quietTrackingSummaryState.isVisible else { return AnyView(EmptyView()) }
        return AnyView(
            passiveTrackingRail
    ...
    var timelineNeedsReplanTraySection: AnyView {
        guard case .trayVisible(let summary) = overlaySnapshot.replanState.phase else {
    ...
    var timelineMainSurfaceSection: AnyView {
    ...
    var timelineDailyReflectionSection: AnyView {
        guard let entryState = chromeSnapshot.dailyReflectionEntryState else { return AnyView(EmptyView()) }
        return AnyView(
            timelineColumnContent {
    ...
    var timelineFooterModulesSection: AnyView {
        guard let footerContent = timelineFooterModules else { return AnyView(EmptyView()) }
        return footerContent
    }

    var timelineGuidanceSection: AnyView {
        guard let guidanceState = overlaySnapshot.guidanceState else { return AnyView(EmptyView()) }
        return AnyView(
    ...
    var timelinePersistentReplanEntrySection: AnyView {
        AnyView(
            timelineColumnContent {
    ...
    var timelineDaySwipeGestureSurface: some View {
