...
struct SunriseDaySwipeHandleInteractionState: Equatable {
...
struct SunriseDaySwipeOverlay: View {
    let isEnabled: Bool
    ...
    let onHandleDragChanged: (SunriseDaySwipeSide, CGSize, CGPoint, CGSize) -> Void
    let onHandleDragEnded: (SunriseDaySwipeSide, CGSize, CGSize, CGPoint, CGSize) -> Void

    @Binding var leadingData: SunriseDaySwipeData
    @Binding var trailingData: SunriseDaySwipeData
    @Binding var topSide: SunriseDaySwipeSide
    @State private var lastDraggedHandleSide: SunriseDaySwipeSide?
    @State private var lastHandleDragEndedAt: Date?
    @State private var handleInteractionState = SunriseDaySwipeHandleInteractionState()
    ...
    var body: some View {
        ...
        .animation(reduceMotion ? nil : .easeInOut(duration: 0.22), value: isChromeVisible)
        .accessibilityHidden(!isEnabled || !isChromeVisible)
    }

    private func sideLayer(_ side: SunriseDaySwipeSide, size: CGSize) -> some View {
        ...
        return ZStack(alignment: .topLeading) {
            waveGlassLayer(data: data)
                .opacity(isEnabled ? presentation.opacity : 0)
            ...
            Button {
                ...
                Label(side.accessibilityLabel, systemImage: side.systemImage)
                    .labelStyle(.iconOnly)
                    .font(.system(size: 14 * SunriseDaySwipeData.buttonVisualScale, weight: .bold))
                    .symbolRenderingMode(.hierarchical)
                    .foregroundStyle(Color.lifeboard.accentOnPrimary)
                    .frame(
                        width: SunriseDaySwipeData.buttonVisualRadius * 2,
                        height: SunriseDaySwipeData.buttonVisualRadius * 2
                    )
                    .background {
                        Circle()
                    ...
                    .frame(
                        width: SunriseDaySwipeData.buttonRadius * 2,
                        height: SunriseDaySwipeData.buttonRadius * 2
                    )
                    .contentShape(Circle())
            }
            .buttonStyle(.plain)
    ...
    @ViewBuilder
    private func waveGlassLayer(data: SunriseDaySwipeData) -> some View {
        let shape = SunriseDaySwipeWaveShape(data: data)
    ...
    private func handleDragGesture(for side: SunriseDaySwipeSide, size: CGSize) -> some Gesture {
    ...
    private func handleDragLocation(
        ...
        let center = SunriseDaySwipeData(
            side: side,
            containerSize: size,
    ...
    private func commit(_ side: SunriseDaySwipeSide, size: CGSize) {
        guard isEnabled, isChromeVisible else { return }
        let interval = LifeBoardPerformanceTrace.begin("SunriseDaySwipeCommit")
    ...
    private func shouldCommitTap(for side: SunriseDaySwipeSide) -> Bool {
    ...
    private func reset(_ side: SunriseDaySwipeSide, size: CGSize) {
    ...
    private func swipeData(for side: SunriseDaySwipeSide, size: CGSize) -> SunriseDaySwipeData {
    ...
    private func setSwipeData(_ data: SunriseDaySwipeData) {
        switch data.side {
        case .leading:
            leadingData = data
        case .trailing:
...
struct SunriseDaySwipeGestureSurface: UIViewRepresentable {
    let isEnabled: Bool
    let containerSize: CGSize
    let restingCenterY: CGFloat
    let resolver: HomeDaySwipeResolver
    let onInteractionStarted: () -> Void
    let onChanged: (SunriseDaySwipeSide, CGSize, CGPoint) -> Void
    let onEnded: (SunriseDaySwipeSide, CGSize, CGSize, CGPoint) -> Void
    let onCancelled: (SunriseDaySwipeSide) -> Void
    ...
    final class Coordinator: NSObject, UIGestureRecognizerDelegate {
        var parent: SunriseDaySwipeGestureSurface

        private weak var scrollView: UIScrollView?
        private weak var installedView: UIView?
        private weak var panRecognizer: UIPanGestureRecognizer?
        private var activeSide: SunriseDaySwipeSide?
        private var hasStartedInteraction = false

        init(parent: SunriseDaySwipeGestureSurface) {
            self.parent = parent
        }
        ...
        private func isHandleLocation(_ location: CGPoint, in size: CGSize) -> Bool {
            let hitRadius = SunriseDaySwipeData.buttonRadius + 12
            return SunriseDaySwipeSide.allCases.contains { side in
                let center = SunriseDaySwipeData(
                    side: side,
                    containerSize: size,
