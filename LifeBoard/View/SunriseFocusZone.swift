...
public struct SunriseFocusZone: View {
...
enum SunriseFocusZoneBadgeTone: Equatable {
    case danger
    case warning
    case success
...
struct SunriseFocusZoneBadgePresentation: Equatable {
    let text: String
    let tone: SunriseFocusZoneBadgeTone
}

struct SunriseFocusZoneSecondarySegment: Equatable {
    enum Kind: Equatable {
        case urgency(SunriseFocusZoneBadgeTone)
        case metadata
    }
...
private enum SunriseFocusZoneSecondaryFormatting {
    static let separator = " · "
}

struct SunriseFocusZoneRowPresentation: Equatable {
    let title: String
    let secondarySegments: [SunriseFocusZoneSecondarySegment]

    var secondaryLineText: String? {
        guard secondarySegments.isEmpty == false else { return nil }
        return secondarySegments.map(\.text).joined(separator: SunriseFocusZoneSecondaryFormatting.separator)
    }

    static func make(task: TaskDefinition, insight: EvaFocusTaskInsight?, now: Date = Date()) -> SunriseFocusZoneRowPresentation {
        _ = insight
        let timePressure = SunriseFocusZoneTimePressureResolver.resolve(
            task: task,
            now: now,
            showDueTodayTime: false
        )
        let metadata = SunriseFocusZoneSecondaryLineResolver.resolve(
            task: task,
            showInboxProjectName: false
        )
        var secondarySegments: [SunriseFocusZoneSecondarySegment] = []

        if let timePressure {
            secondarySegments.append(
                SunriseFocusZoneSecondarySegment(
        ...
        return SunriseFocusZoneRowPresentation(
            title: task.title,
...
enum SunriseFocusZoneTimePressureResolver {
    static func resolve(
        task: TaskDefinition,
        now: Date = Date(),
        showDueTodayTime: Bool = true
    ) -> SunriseFocusZoneBadgePresentation? {
        ...
        if let dueDate = task.dueDate, let lateAgeToken = OverdueAgeFormatter.lateAgeToken(dueDate: dueDate, now: now) {
            return SunriseFocusZoneBadgePresentation(text: "Late by \(lateAgeToken)", tone: .danger)
        }

        if isDueSoon(task: task, now: now) {
            return SunriseFocusZoneBadgePresentation(text: "Due soon", tone: .warning)
        }
        ...
        return SunriseFocusZoneBadgePresentation(
...
struct SunriseFocusZoneSecondaryLine: Equatable {
    let text: String?
}

enum SunriseFocusZoneSecondaryLineResolver {
    static func resolve(
        task: TaskDefinition,
        showInboxProjectName: Bool = true
    ) -> SunriseFocusZoneSecondaryLine {
        if task.projectID == ProjectConstants.inboxProjectID {
            guard showInboxProjectName else {
                return SunriseFocusZoneSecondaryLine(text: nil)
            }
            return SunriseFocusZoneSecondaryLine(text: "Inbox")
        }
        ...
        if let projectName, !projectName.isEmpty {
            return SunriseFocusZoneSecondaryLine(text: projectName)
        }
...
private struct SunriseFocusZoneRow: View {
    ...
    private var spacing: LifeBoardSpacingTokens { LifeBoardThemeManager.shared.currentTheme.tokens.spacing }
    private var presentation: SunriseFocusZoneRowPresentation {
        SunriseFocusZoneRowPresentation.make(task: task, insight: insight)
    }
    ...
    @MainActor
    private func styledSecondaryText(for segment: SunriseFocusZoneSecondarySegment) -> Text {
        switch segment.kind {
...
#if DEBUG
struct SunriseFocusZone_Previews: PreviewProvider {
