...
struct ScheduleScreen: View {
    ...
    @State private var scheduleScrollTraceInterval: PerformanceInterval?
    @State private var eventDetailOpenTraceInterval: PerformanceInterval?
    ...
    private var nextUpSecondaryLine: String {
        if let nextMeeting = service.snapshot.nextMeeting {
            return CalendarPresentation.timeRangeText(for: nextMeeting.event)
        }
    ...
    private func recordScheduleScrollActivity() {
        if scheduleScrollTraceInterval == nil {
            scheduleScrollTraceInterval = PerformanceTrace.begin("ScheduleScrollSession")
        }
    }

    private func beginEventDetailOpenTrace() {
        finishEventDetailOpenTraceIfNeeded()
        eventDetailOpenTraceInterval = PerformanceTrace.begin("ScheduleEventDetailOpen")
    }

    private func finishEventDetailOpenTraceIfNeeded() {
        if let eventDetailOpenTraceInterval {
            PerformanceTrace.end(eventDetailOpenTraceInterval)
    ...
    private func finishScheduleScrollTraceIfNeeded() {
        if let scheduleScrollTraceInterval {
            PerformanceTrace.end(scheduleScrollTraceInterval)
            self.scheduleScrollTraceInterval = nil
        }
    }
}

private struct ScheduleHeaderChrome: View {
    ...
    private var contextLine: String {
        ...
        return "\(prefix) • \(CalendarPresentation.scheduleDateText(for: selectedDate))"
    }
