...
struct DayCompassCard: View {
    ...
    var body: some View {
        ...
        ZStack {
            GlassCard(
                ...
                .clipShape(RoundedRectangle(cornerRadius: RadiusTokens.largeCard, style: .continuous))
            }
            .id(model.state.flow)
    ...
    private func decorAsset(for state: DayCompassState) -> DecorAsset {
        switch state {
        case .rescue:
            return .rescueShield
        case .replan:
