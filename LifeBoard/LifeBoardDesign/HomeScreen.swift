...
struct HomeScreen: View {
    ...
    @State private var selectedContentScope: HomeContentScope = .all
    @State private var headerActivationID = TimeOfDayHeaderAsset.makeActivationID()
    @State private var leadingDaySunriseSwipeData = DaySwipeData(side: .leading)
    @State private var trailingDaySunriseSwipeData = DaySwipeData(side: .trailing)
    @State private var topDaySunriseSwipeSide: DaySwipeSide = .trailing
    @State private var activeDaySunriseSwipeSide: DaySwipeSide?
            ...
            HeaderView(
                ...
                DateHeroHeader(
                    model: DateHeroHeader.Model(
                        ...
                        navigatorTitle: HeaderTimeContext.navigatorTitle(selectedDate: chrome.selectedDate, now: timeline.date),
    ...
    private func lifeAreaTodayTimelineRows(lifeAreaID: UUID, now: Date) -> [TimelineRow] {
    ...
    private var streamEmptyStateModel: (model: EmptyState.Model, action: () -> Void) {
                ...
                LazyVStack(spacing: LBSpacingTokens.sm) {
                    ...
                    case .anchor(let anchor):
                        ...
                        TimelineItem(
                            ...
                            TimelineCard(
                                model: TimelineCard.Model(
                                    id: anchor.id,
                                    title: anchorTitle(for: anchor),
                                    subtitle: anchor.subtitle ?? routineSubtitle(for: HeaderTimeContext.resolve(selectedDate: chrome.selectedDate).period),
                                    timeText: timeText(anchor.time),
                                    role: anchorRole,
                                    kind: .anchor,
                                    tintHex: nil,
                    ...
                    case .item(let item):
                        ...
                        let taskToggleAction: (() -> Void)? = item.taskID == nil ? nil : {
                            let interval = PerformanceTrace.begin("HomeTimelineTaskToggle")
                    ...
                    case .meetingFlock(let model, let sourceItems):
                        TimelineItem(
                            ...
                            MeetingFlockCard(model: model) { meeting in
    ...
    private var timelineEmptyStateModel: (model: EmptyState.Model, action: () -> Void) {
    ...
    func timelineRows(now: Date) -> [TimelineRow] {
        let interval = PerformanceTrace.begin("HomeTimelineRowsBuild")
    ...
    nonisolated static func buildTimelineRows(
        ...
        now: Date,
        isToday: Bool,
        contentScope: HomeContentScope = .all,
        meetingFlockModel: ([TimelinePlanItem]) -> MeetingFlockCard.Model
    ) -> [TimelineRow] {
    ...
    private var visibleHabitRows: [HomeHabitRow] {
        let interval = PerformanceTrace.begin("HomeHabitModelsBuild")
        defer { PerformanceTrace.end(interval) }
        return Array(
            (
    ...
    private var habitRowModels: [HabitGridRowModel] {
        visibleHabitRows.map(habitRowModel(for:))
    }

    private func habitRowModel(for row: HomeHabitRow) -> HabitGridRowModel {
        let interaction = HomeHabitLastCellInteraction.resolve(for: row)
        return HabitGridRowModel(
            habitID: row.habitID,
            sourceRow: row,
            title: row.title,
    ...
    private func habitCellModel(for row: HomeHabitRow) -> HabitCell.Model {
        ...
        return HabitCell.Model(
            ...
            cells: cells.map(HabitCell.CellState.init),
            allowsTwoLineTitle: true
        )
    }

    private var permissionModel: (model: PermissionCard.Model, primaryAction: () -> Void, secondaryAction: (() -> Void)?)? {
    ...
    private var syncErrorModel: (model: PermissionCard.Model, primaryAction: () -> Void, secondaryAction: (() -> Void)?)? {
    ...
    private func cardKind(for item: TimelinePlanItem) -> TimelineCard.Kind {
    ...
    private func spineIconSystemName(for item: TimelinePlanItem, kind: TimelineCard.Kind) -> String {
        if kind == .calendar {
            return "calendar"
        }
    ...
    nonisolated static func sortedRows(_ rows: [TimelineRow], now: Date) -> [TimelineRow] {
    ...
    nonisolated static func nextUpcomingCalendarItemID(in rows: [TimelineRow], now: Date) -> String? {
    ...
    private func daySunriseSwipeRestingCenterY(safeAreaTop: CGFloat) -> CGFloat {
        max(safeAreaTop, 0) + DaySwipeData.timelineHandleCenterY
    }

    private func daySunriseSwipeData(
        for side: DaySwipeSide,
        size: CGSize,
    ...
    private func setDaySunriseSwipeData(_ data: DaySwipeData) {
        switch data.side {
        case .leading:
    ...
    private func setScrolling(_ active: Bool) {
        ...
        PerformanceTrace.event("HomeScrollActive", value: active ? 1 : 0)
        PerformanceTrace.event("HomeFlattenedRenderMode", value: active ? 1 : 0)
    }
    ...
    private func updateDaySunriseSwipeChromeVisibility(for state: HomeScrollChromeState) {
        let nextVisibility = DaySwipeChromeVisibilityPolicy.nextVisibility(
...
#if canImport(UIKit)
private struct HomeScrollChromeObserver: UIViewRepresentable {
    let isEnabled: Bool
    ...
    final class Coordinator: NSObject, UIGestureRecognizerDelegate {
        var parent: HomeScrollChromeObserver
        ...
        init(parent: HomeScrollChromeObserver) {
            self.parent = parent
        }
...
enum HomeContentScope: String, CaseIterable, Sendable {
    case all
    case meetings
    case tasks
    case habits
...
private struct HabitGridRowModel: Identifiable, Equatable {
    let habitID: UUID
    let sourceRow: HomeHabitRow
    let title: String
    let cellModel: HabitCell.Model
...
private struct HabitGridCard: View, Equatable {
    let rows: [HabitGridRowModel]
    let onOpenHabitBoard: () -> Void
    let onCycleHabit: (HabitGridRowModel) -> Void
    let onAddHabit: () -> Void
    @Environment(\.lifeboardScrollOptimizedRendering) private var scrollOptimizedRendering

    nonisolated static func == (lhs: HabitGridCard, rhs: HabitGridCard) -> Bool {
        lhs.rows == rhs.rows
    }
            ...
            GlassCard(cornerRadius: RadiusTokens.largeCard) {
                VStack(spacing: LBSpacingTokens.md) {
                    ...
                    if rows.isEmpty {
                        ...
                        ForEach(rows) { row in
                            HabitGridRow(model: row, onCycleHabit: onCycleHabit)
                                .equatable()
                        }
                    }
                }
...
private struct HabitGridRow: View, Equatable {
    let model: HabitGridRowModel
    let onCycleHabit: (HabitGridRowModel) -> Void

    nonisolated static func == (lhs: HabitGridRow, rhs: HabitGridRow) -> Bool {
        lhs.model == rhs.model
    }

    var body: some View {
        Button {
            let interval = PerformanceTrace.begin("HomeHabitLastCellTap")
            ...
            HabitCell(model: model.cellModel)
                .equatable()
                .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
...
enum TimelineRow: Identifiable {
    ...
    case meetingFlock(MeetingFlockCard.Model, [TimelinePlanItem])
    case now(Date)
