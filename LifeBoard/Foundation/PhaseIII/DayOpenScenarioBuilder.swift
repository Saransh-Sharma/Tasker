...
public enum DayOpenScenarioBuilder {
    ...
    // Proposal fidelity is intentionally not encoded into this authoritative
    // scenario payload. `DayCloseStore` records it after apply in the local-only
    // `DayOpenProposalSignalStore`, keyed by the resulting receipt ID. That
    // preserves source-string and payload compatibility while letting Undo
    // exclude evidence by joining sidecars only to currently applied receipts.
