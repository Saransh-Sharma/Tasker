...
final class SunriseFocusZoneStatusTests: XCTestCase {
    func testSunriseFocusZoneRowPresentationMakePrioritizesLateOverNonUrgencySignals() {
    ...
    func testSunriseFocusZoneRowPresentationMakeShowsDueSoonWhenTaskIsApproachingDeadline() {
        let now = Calendar.current.date(from: DateComponents(year: 2026, month: 2, day: 24, hour: 9, minute: 0))!
        let task = makeTask(
            title: "Soon task",
    ...
    func testSunriseFocusZoneRowPresentationMakeDoesNotSurfaceQuickWinWithoutUrgency() {
        let now = Calendar.current.date(from: DateComponents(year: 2026, month: 2, day: 24, hour: 9, minute: 0))!
        let task = makeTask(
            title: "Quick task",
    ...
    func testSunriseFocusZoneRowPresentationMakeDoesNotSurfaceUnblockedWithoutUrgency() {
        let now = Calendar.current.date(from: DateComponents(year: 2026, month: 2, day: 24, hour: 9, minute: 0))!
        let task = makeTask(
    ...
    func testSunriseFocusZoneRowPresentationMakeReturnsNilForCompletedTask() {
        let now = Calendar.current.date(from: DateComponents(year: 2026, month: 2, day: 24, hour: 9, minute: 0))!
        var task = makeTask(
            title: "Done task",
