...
func makeContainer(repoRoot: URL) throws -> NSPersistentContainer {
    let sourceModel = repoRoot.appendingPathComponent("To Do List/TaskModelV2.xcdatamodeld")
    let tempDir = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
    let compiledModel = tempDir.appendingPathComponent("TaskModelV2-benchmark-\(UUID().uuidString).momd")
    ...
    guard let model = NSManagedObjectModel(contentsOf: compiledModel) else {
        throw NSError(domain: "perf_seed_v2", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to load compiled model"])
    }
