...
//        backdropBackgroundImageView.frame =  CGRect(x: 0, y: backdropNochImageView.bounds.height, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height)
    ...
    @objc func showCalMoreButtonnAction() {
        ...
        if (isProjectsDown) {
            UIView.animate(withDuration: duration, delay: delay, usingSpringWithDamping: 0.5, initialSpringVelocity: 2, options: .curveLinear, animations: {
                self.moveUp_toHideProjects(view: self.foredropContainer)
            }) { (_) in
        ...
        } else if (isCalDown && isChartsDown) {
            self.calendar.isHidden = true
            isCalDown = false
    ...
    @objc func showProjectsButtonAction() {
        let delay: Double = 0.1
        let duration: Double = 0.8

        if (!isProjectsDown && (!isCalDown || !isChartsDown)) {
            if (isCalDown) {
                UIView.animate(withDuration: duration, delay: 0.1, usingSpringWithDamping: 0.5, initialSpringVelocity: 2, options: .curveLinear, animations: {
                    self.moveUp_toHideCal(view: self.foredropContainer)
                }) { [self] (_) in
                    UIView.animate(withDuration: duration, delay: delay, usingSpringWithDamping: 0.5, initialSpringVelocity: 2, options: .curveLinear, animations: {
                        self.moveDown_revealProjects(view: self.foredropContainer)
                    }) { (_) in
                    }
                    isProjectsDown = true
                }
        ...
        } else if (isProjectsDown && (!isCalDown || !isChartsDown)) {
            UIView.animate(withDuration: duration, delay: delay, usingSpringWithDamping: 0.5, initialSpringVelocity: 2, options: .curveLinear, animations: {
                self.moveUp_toHideProjects(view: self.foredropContainer)
            }) { (_) in
            ...
            isProjectsDown = false
        } else if ((isCalDown || isChartsDown) && !isProjectsDown) {
            if (isCalDown && isChartsDown) {
                UIView.animate(withDuration: duration, delay: delay, usingSpringWithDamping: 0.5, initialSpringVelocity: 2, options: .curveLinear, animations: {
