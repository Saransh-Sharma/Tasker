...
public enum StateTaskMapper {
    ...
    public static func updateEntity(_ entity: NTask, from task: Task) {
        ...
        entity.notes = task.details
        entity.taskDetails = task.details
        entity.status = task.isComplete ? "completed" : "pending"
        ...
        entity.createdBy = entity.createdBy ?? UIDevice.current.name
        entity.createdAt = (entity.createdAt as Date?) == nil ? Date() as NSDate : entity.createdAt
    ...
    public static func toDomain(from entity: NTask) -> Task {
        let id = entity.taskID ?? entity.id ?? UUID()
        ...
        let priorityRaw = entity.priority > 0 ? entity.priority : entity.taskPriority
        let energy = TaskEnergy(rawValue: entity.energy ?? "") ?? .medium
        let category = TaskCategory(rawValue: entity.category ?? "") ?? .general
        let context = TaskContext(rawValue: entity.context ?? "") ?? .anywhere
        let details = entity.notes ?? entity.taskDetails

        return Task(
            id: id,
            projectID: projectID,
            name: entity.title ?? entity.name ?? "Untitled Task",
            details: details,
