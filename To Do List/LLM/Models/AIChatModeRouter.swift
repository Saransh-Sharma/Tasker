...
struct AIChatModeRouter {
    ...
    static func route(for feature: AIFeatureRoute, appManager: AppManager) -> AIModelRoute {
        ...
        let preferredFallbacks = fallbackCandidates(for: feature)
        let installed = Set(appManager.installedModels)
        let maxModelSize = maxSupportedModelSizeGB(for: appManager)
        let idealConfig = ModelConfiguration.getModelByName(ideal)
        let idealSize = idealConfig?.modelSize
        let canUseIdeal = isAllowedByDeviceBudget(modelName: ideal, appManager: appManager, maxModelSize: maxModelSize)
        ...
        if let fallback = preferredFallbacks.first(where: {
            installed.contains($0) && isAllowedByDeviceBudget(modelName: $0, appManager: appManager, maxModelSize: maxModelSize)
        }) {
        ...
        if let current = appManager.currentModelName,
           isAllowedByDeviceBudget(modelName: current, appManager: appManager, maxModelSize: maxModelSize) {
    ...
    static func idealModelName(for feature: AIFeatureRoute) -> String {
        ...
        case .planMode, .topThree, .breakdown:
            return ModelConfiguration.deepseek_r1_distill_qwen_1_5b_4bit.name
        }
    }

    /// Executes fallbackCandidates.
    static func fallbackCandidates(for feature: AIFeatureRoute) -> [String] {
        switch feature {
        case .addTaskSuggestion, .dynamicChips, .dailyBrief:
            return [
        ...
        case .planMode, .topThree, .breakdown:
            return [
                ...
                ModelConfiguration.qwen_3_0_6b_4bit.name,
                ModelConfiguration.llama_3_2_3b_4bit.name,
                ModelConfiguration.deepseek_r1_distill_qwen_1_5b_8bit.name,
                ModelConfiguration.qwen_3_4b_4bit.name
            ]
        }
    }
    ...
    private static func maxSupportedModelSizeGB(for appManager: AppManager) -> Decimal {
        let available = appManager.availableMemory
    ...
    private static func isAllowedByDeviceBudget(
        ...
        _ = appManager // keep signature explicit for future routing heuristics
