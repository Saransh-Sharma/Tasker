...
Make It Fit Today, Friction Detective, and Weekly Reset with EVA now exist as local-first product rituals that default on in both app configurations. Their promoted local flags and signed runtime switches must both permit presentation, preserving three independent remote kill switches without making offline use depend on Cloud EVA. They use canonical capacity and weekly planning, actionable evidence, inline estimate repair, explicit partial-save receipts, IDs-only draft restoration, local-only structured friction findings, corrected Carry/Later/Release semantics, seven-day friction outcome capture, and deterministic Undo. Weekly planning changes use the existing durable assistant action-run pipeline; Make It Fit restores its canonical Plan receipt by identifier. Friction Detective prevents duplicate application after restoration and only exposes its task inverse while the in-memory receipt is available, rather than persisting a sensitive full task snapshot in navigation state. See [EVA Decision Loops implementation](EVA_DECISION_LOOPS_IMPLEMENTATION.md).
