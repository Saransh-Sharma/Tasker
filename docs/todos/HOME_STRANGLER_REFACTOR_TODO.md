...
- [x] Last verified slice: moved Home search state and engine support into `Presentation/Home/Search/HomeSearchState.swift`.
- [x] Latest structural checks passed: Home search status filter, request signature, focus policy resolver, engine protocol/adapter, refresh coordinator, and state live only in the new search support file; `SunriseAppShellView.swift` keeps only usages.
...
- [x] Latest focused Home tests passed: `191 passed, 0 failed`.
- [ ] Known non-blocking noise remains: app extension `CFBundleVersion` mismatch, multiple matching simulator destination warning, and existing Swift concurrency/deprecation warnings in tests.
...
- [ ] Split Home shell/controller extensions.
  ...
  - [ ] Move bindings, render pipeline, keyboard, lifecycle, bottom bar mounting, and shell construction extensions.
- [ ] Split Home navigation, modals, iPad construction, and UI test seeding.
  ...
  - [ ] Move navigation/deep-link/reload delegate extensions, modal presentation methods, and iPad helpers.
