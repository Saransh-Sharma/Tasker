...
Every living product document includes classification, audience, capability status, source authority, and last verified date. Update the catalog and at least one user journey when a capability changes. Run `scripts/check-documentation-current.sh` before merging documentation changes.
