...
public enum CalendarAccessAction: Equatable {
    ...
    case unavailable(CalendarAuthorizationStatus)
    case noneNeeded
}
...
public struct CalendarAccessAttemptRecord: Codable, Equatable {
    ...
    public let statusBefore: CalendarAuthorizationStatus
    public let statusAfter: CalendarAuthorizationStatus?
    ...
    public init(
        ...
        source: String,
        statusBefore: CalendarAuthorizationStatus,
        statusAfter: CalendarAuthorizationStatus?,
...
public final class CalendarIntegrationService: ObservableObject {
    ...
    private var contextEvents: [CalendarEventSnapshot] = []
    private var contextEventFetchRange: ClosedRange<Date>?
    private var refreshGeneration: UInt64 = 0
    private var projectionRevision: UInt64 = 0
    private var dayProjectionCache: [DayProjectionKey: [CalendarEventSnapshot]] = [:]
    private var weekProjectionCache: [WeekProjectionKey: [CalendarDayAgenda]] = [:]
    ...
    public init(
        ...
        NotificationCenter.default.publisher(for: WorkspacePreferencesStore.didChangeNotification)
    ...
    public func accessAction(for status: CalendarAuthorizationStatus) -> CalendarAccessAction {
        switch status {
        case .notDetermined:
            ...
            logCalendarWarning(
                ...
                fields: calendarDiagnosticFields(
                    ...
                    extra: [
                        ...
                        "statusAfter": CalendarAuthorizationStatus.denied.rawValue,
    ...
    public func updateSelectedCalendarIDs(_ calendarIDs: [String]) {
        let normalizedCalendarIDs = WorkspacePreferences.normalizeSelectedCalendarIDs(calendarIDs)
    ...
    public func refreshContext(referenceDate: Date = Date(), reason: String = "manual") {
        ...
        let weekStart = XPCalculationService.startOfWeek(
            for: referenceDate,
    ...
    public func eventsForDay(_ day: Date) -> [CalendarEventSnapshot] {
        let calendar = Calendar.current
    ...
    public func weekAgenda(anchorDate: Date, weekStartsOn: Weekday) -> [CalendarDayAgenda] {
        let weekStart = XPCalculationService.startOfWeek(for: anchorDate, startingOn: weekStartsOn)
    ...
    @MainActor
    public func taskFitHint(for task: TaskDefinition, now: Date = Date()) -> TaskFitHintResult {
    ...
    public func event(withID eventID: String) -> CalendarEventSnapshot? {
    ...
    private func reloadPreferencesAndRefresh(reason: String) {
        ...
        snapshot.selectedCalendarIDs = WorkspacePreferences.normalizeSelectedCalendarIDs(preferences.selectedCalendarIDs)
