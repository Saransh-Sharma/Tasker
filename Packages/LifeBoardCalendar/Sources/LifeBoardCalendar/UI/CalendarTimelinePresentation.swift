...
public enum CalendarTimelinePlanner {
    ...
    public static func makePlan(
        ...
        let positionedEvents = clusters.flatMap { cluster -> [CalendarTimelineLayoutPlan.PositionedEvent] in
            var laneEndMinutes: [Int] = []
            var positionedCluster: [CalendarTimelineLayoutPlan.PositionedEvent] = []
            ...
            for event in cluster {
                if let reusableLane = laneEndMinutes.firstIndex(where: { $0 <= event.startMinute }) {
                    ...
                    positionedCluster.append(
                        CalendarTimelineLayoutPlan.PositionedEvent(
                            event: event.event,
                            lane: reusableLane,
                            laneCount: 0,
                            columnSpan: 1,
            ...
            return positionedCluster.map { positioned in
                ...
                return CalendarTimelineLayoutPlan.PositionedEvent(
        ...
        return CalendarTimelineLayoutPlan(
    ...
    private static func maxColumnSpan(
        for event: CalendarTimelineLayoutPlan.PositionedEvent,
        in cluster: [CalendarTimelineLayoutPlan.PositionedEvent],
        laneCount: Int
    ) -> Int {
...
struct CalendarTimelinePlanEventSignature: Equatable {
    let id: String
...
struct CalendarTimelinePlanCacheKey: Equatable {
    let date: Date
    let density: CalendarTimelineDensity
    let visibleWindowPolicy: CalendarTimelineVisibleWindowPolicy
    let calendarIdentifier: Calendar.Identifier
    let calendarTimeZone: TimeZone
    let events: [CalendarTimelinePlanEventSignature]

    init(
        events: [CalendarEventSnapshot],
        date: Date,
        density: CalendarTimelineDensity,
        visibleWindowPolicy: CalendarTimelineVisibleWindowPolicy,
        calendar: Calendar = .current
    ) {
        ...
        self.events = events.map(CalendarTimelinePlanEventSignature.init)
    }
}
...
public struct CalendarTimelinePlanCache {
    ...
    public mutating func plan(
        ...
        let key = CalendarTimelinePlanCacheKey(
            events: events,
            date: date,
            density: density,
        ...
        let plan = CalendarTimelinePlanner.makePlan(
            for: events,
            on: date,
            density: density,
...
public struct CalendarTimelineView: View {
    ...
    let onSelectEvent: ((CalendarEventSnapshot) -> Void)?
    ...
    public init(
        ...
        onSelectEvent: ((CalendarEventSnapshot) -> Void)? = nil
    ) {
        self.date = date
        self.events = events
        self.density = density
    ...
    private func timelineRows(
        for plan: CalendarTimelineLayoutPlan,
        anchorDate: Date
    ) -> some View {
    ...
    private func eventButton(_ event: CalendarEventSnapshot) -> some View {
        Button {
            onSelectEvent?(event)
        } label: {
            CalendarTimelineEventRow(event: event, density: density)
        }
        .buttonStyle(.plain)
        .disabled(onSelectEvent == nil)
        .accessibilityIdentifier("\(eventAccessibilityIdentifierPrefix).\(event.id)")
        .accessibilityLabel("\(event.title), \(CalendarPresentation.timeRangeText(for: event))")
    }
    ...
    private func refreshLayoutPlan(anchorDate: Date) {
        let policy = CalendarTimelineVisibleWindowPolicy.fixedToCurrentMinute(anchorDate)
    ...
    private func events(
        in hour: Int,
        plan: CalendarTimelineLayoutPlan
    ) -> [CalendarTimelineLayoutPlan.PositionedEvent] {
...
private struct CalendarTimelineEventRow: View {
    let event: CalendarEventSnapshot
    let density: CalendarTimelineDensity

    private var accentColor: Color {
        HexColor.color(event.calendarColorHex, fallback: Color.lifeboard.accentPrimary)
    }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: density == .compact ? 8 : 10) {
            ...
            VStack(alignment: .leading, spacing: density == .compact ? 2 : 4) {
                ...
                Text(CalendarPresentation.timeRangeText(for: event))
                    .font(.lifeboard(.caption2))
                    .foregroundStyle(Color.lifeboard.textSecondary)
                    .lineLimit(1)
