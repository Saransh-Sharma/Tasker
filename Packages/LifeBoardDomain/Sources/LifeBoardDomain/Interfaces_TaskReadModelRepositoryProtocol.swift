...
public extension TaskReadModelRepositoryProtocol {
    ...
    func fetchWeekChartProjection(
        ...
        var mutableCalendar = Calendar.current
        mutableCalendar.firstWeekday = 1
        // Immutable copy: the completion closure below runs concurrently and
        // cannot capture a `var`.
        let calendar = mutableCalendar
        // Spelled out rather than using the app's `Calendar.daysWithSameWeekOfYear`
        // / `Date.startOfDay` / `Date.endOfDay` helpers: those live in
        // LifeBoard/Utils and this package must not depend on the app.
        let weekOfYear = calendar.component(.weekOfYear, from: referenceDate)
        let yearForWeek = calendar.component(.yearForWeekOfYear, from: referenceDate)
        let firstDayOfWeek = calendar.date(
            from: DateComponents(weekOfYear: weekOfYear, yearForWeekOfYear: yearForWeek)
        )
        let week: [Date] = firstDayOfWeek.map { start in
            (0..<7).compactMap { calendar.date(byAdding: .day, value: $0, to: start) }
        } ?? []
        guard let weekStart = week.first.map({ calendar.startOfDay(for: $0) }),
        ...
        fetchTasks(
            ...
            case .success(let slice):
                ...
                for task in slice.tasks where task.isComplete {
                    guard let completedDay = task.dateCompleted.map({ calendar.startOfDay(for: $0) }) else { continue }
