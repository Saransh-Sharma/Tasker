...
public protocol DomainEventUseCase {
...
public final class DomainEventPublisher: ObservableObject {
    ...
    private var eventHandlers: [DomainEventUseCase] = []
    ...
    public func publish(_ event: DomainEvent) {
        ...
        DomainDiagnostics.debugLog?("📤 Domain Event Published: \(event.eventType) for aggregate \(event.aggregateId)")
    }
    
    /// Register an event handler
    public func register(handler: DomainEventUseCase) {
        eventHandlers.append(handler)
        DomainDiagnostics.debugLog?("📝 Registered event handler for events: \(type(of: handler))")
    }
    
    /// Unregister an event handler
    public func unregister(handler: DomainEventUseCase) {
    ...
    public func clearEventStorage() {
        ...
        DomainDiagnostics.debugLog?("🗑️ Event storage cleared")
    }
    ...
    private func logEvent(_ event: DomainEvent) {
        ...
        DomainDiagnostics.debugLog?("📋 [\(timestamp)] \(event.eventType) - Aggregate: \(event.aggregateId.uuidString.prefix(8))")
    }
}
...
public final class AnalyticsEventUseCase: DomainEventUseCase {
    ...
    private func recordTaskCompletion(_ event: TaskCompletedEvent) {
        ...
        DomainDiagnostics.debugLog?("📊 Analytics: Task completed - Score: \(event.scoreEarned)")
    }
    
    /// Executes recordTaskCreation.
    private func recordTaskCreation(_ event: TaskCreatedEvent) {
        // Record analytics for task creation
        DomainDiagnostics.debugLog?("📊 Analytics: Task created - Priority: \(event.taskPriority.displayName)")
    }
    ...
    private func recordProjectCreation(_ event: ProjectCreatedEvent) {
        ...
        DomainDiagnostics.debugLog?("📊 Analytics: Project created - \(event.projectName)")
    }
}

/// Notification event handler
public final class NotificationEventUseCase: DomainEventUseCase {
...
/// Diagnostics hook for the domain layer.
///
/// `DomainEventPublisher` logged through the app's `logDebug`, which a package
/// cannot reach. The app installs this at launch; unset, the domain layer is
/// silent. Same shape as `TokensDiagnostics`.
public enum DomainDiagnostics {
