...
public enum NotificationCategoryID: String, CaseIterable, Sendable {
...
public enum LocalNotificationKind: String, Codable, CaseIterable, Sendable {
    case taskReminder
    case dueSoon
    case overdue
    case morningPlan
    ...
    public var defaultCategoryID: NotificationCategoryID {
        switch self {
...
public enum NotificationActionID: String, Codable, CaseIterable, Sendable {
...
public enum NotificationAuthorizationStatus: String, Equatable, Sendable {
    case notDetermined
    case denied
    case authorized
    case provisional
...
public enum DailySummaryKind: String, Equatable, Codable, Sendable {
    case morning
    case nightly
}

public enum NotificationRoute: Equatable, Codable, Sendable {
    ...
    case dailySummary(kind: DailySummaryKind, dateStamp: String?)
    ...
    public static func from(payload: String, fallbackTaskID: UUID?) -> NotificationRoute {
        ...
        if payload.hasPrefix("daily_summary:") {
            ...
            if let kind = DailySummaryKind(rawValue: kindRaw) {
                let dateStamp = components.count > 1 ? String(components[1]) : nil
                let normalizedDateStamp = dateStamp?.isEmpty == false ? dateStamp : nil
...
public struct LocalNotificationRequest: Equatable, Sendable {
    ...
    public let kind: LocalNotificationKind
...
public struct PendingNotificationRequest: Equatable, Sendable {
    public let id: String
    public let fireDate: Date?
    public let kind: LocalNotificationKind?
    public let title: String
...
public struct WorkspacePreferences: Codable, Equatable, Sendable {
    ...
    public func normalizedForPersistence() -> WorkspacePreferences {
...
public final class NotificationPreferencesStore: @unchecked Sendable {
    ...
    public func load() -> NotificationPreferences {
        ...
        if let legacyData = defaults.data(forKey: legacyKey),
           let legacyPreferences = try? decoder.decode(NotificationPreferences.self, from: legacyData) {
    ...
    public func update(_ mutate: (inout NotificationPreferences) -> Void) {
        var current = load()
        mutate(&current)
        save(current)
    }
...
public final class WorkspacePreferencesStore: @unchecked Sendable {
    public static let shared = WorkspacePreferencesStore()
    ...
    public func load() -> WorkspacePreferences {
        lock.lock()
        defer { lock.unlock() }
        guard let data = defaults.data(forKey: key),
              let preferences = try? decoder.decode(WorkspacePreferences.self, from: data) else {
            return WorkspacePreferences()
        }
        return preferences
    }

    public func save(_ preferences: WorkspacePreferences) {
        lock.lock()
        defer { lock.unlock() }
        let normalized = preferences.normalizedForPersistence()
        guard let currentData = defaults.data(forKey: key),
              let current = try? decoder.decode(WorkspacePreferences.self, from: currentData) else {
    ...
    public func update(_ mutate: (inout WorkspacePreferences) -> Void) {
        var current = load()
        mutate(&current)
        save(current)
    }
