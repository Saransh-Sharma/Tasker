...
- **Onboarding:** one question per step over the muted dawn hero video, a personal promise, an immediate CTA, and a single persistent Life Map the whole flow mutates. The map is a **weave**, not a diagram of the product: five faint operating strands sweep through a quiet Now anchor, and the person's own life areas are woven onto them as the only labelled objects. Strand labels appear once, at the reveal, as the map hands off to the dock — before that the roots are structure, not peers of the user's life. Progress is a small discrete track, never a percentage: a life is not a completion bar. The map carries no ambient motion of its own, because the hero video is the screen's one ambient timeline; it moves only when the person changes something, then settles. The persisted map—not sample content—is the payoff and transforms into Home.
